...
export default function OffHireVessels() {
  const { data, error } = useSWR<{ total: number; vessels: Vessel[] }>(
    '/api/dashboard/offhire-vessels',
    fetcher,
  ...
  if (error || !data) {
    return <p className="text-sm text-red-600 mt-6">Failed to load off-hire vessel data.</p>
  }
    ...
    <div className="bg-white shadow-md rounded-xl p-6 mt-6">
      <div className="flex justify-between items-start mb-4">
        <div className="flex items-center space-x-2">
          ...
          <h2 className="text-lg font-semibold text-gray-800">Off-Hire Vessels Today</h2>
        </div>
      ...
      {data.total > 0 ? (
        ...
        <p className="text-sm text-gray-500">No vessels were marked as off-hire during the current window.</p>
      )}
    </div>
  )
}
